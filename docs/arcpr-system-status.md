...
1. **Stale or damaged Arc mount recovery is not automatic.** PR `14422811` failed when `arc mount` found a previous abnormal mount termination (`Device not configured`), then later when its object-store `.arc/port` was absent. A later push failed because the mount was no longer an Arc repository. The runner should unmount stale FUSE state, recreate the object store/mount, and retry preparation before declaring the task failed.
