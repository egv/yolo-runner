...
> **CORRECTION (2026-07-15, post-reinstall):** the reinstall was performed and
> the "5.0.4 kext inside a 5.0.6 bundle" turned out to be **normal**: macFUSE
> 5.0.5 and 5.0.6 ship NO kext changes (see their GitHub release notes), so the
> kernel extension legitimately identifies as 5.0.4 in every 5.0.6 install.
> `kmutil showloaded` printing `(5.0.4)` IS the healthy state for distribution
> 5.0.6, and no approval prompt appears because the identical kext was already
> approved. The kernel panics therefore occurred on the Arc-validated stack;
> the fix is not a version change but reducing FUSE stress (see the last
> section) and reporting the panic to DEVTOOLSSUPPORT / macFUSE upstream.
